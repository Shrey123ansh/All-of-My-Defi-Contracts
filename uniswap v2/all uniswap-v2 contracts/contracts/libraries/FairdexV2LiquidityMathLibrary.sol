...
import "./FullMath.sol";
import "./SafeMath.sol";
import "./FairdexV2Library.sol";

// library containing some math for dealing with the liquidity shares of a pair, e.g. computing their exact value
// in terms of the underlying tokens
library FairdexV2LiquidityMathLibrary {
    ...
    function getReservesAfterArbitrage(
        ...
        (reserveA, reserveB) = FairdexV2Library.getReserves(
            factory,
            tokenA,
            tokenB
        );
        ...
        require(
            ...
            "FairdexV2ArbitrageLibrary: ZERO_PAIR_RESERVES"
        );
        ...
        if (aToB) {
            uint amountOut = FairdexV2Library.getAmountOut(
                amountIn,
                reserveA,
                reserveB
            );
    ...
    function getLiquidityValue(
        ...
        (uint256 reservesA, uint256 reservesB) = FairdexV2Library.getReserves(
            factory,
            tokenA,
            tokenB
        );
        IFairdexV2Pair pair = IFairdexV2Pair(
            FairdexV2Library.pairFor(factory, tokenA, tokenB)
        );
        bool feeOn = IFairdexV2Factory(factory).feeTo() != address(0);
