...
import "./interfaces/IFairdexV2Pair.sol";
import "./interfaces/IFairdexV2Factory.sol";
import "./Swapper.sol";

// BrewFairX is MasterChef's left hand and kinda a wizard. He can brew FairX from pretty much anything!
// This contract handles "serving up" rewards for xFair holders by trading tokens collected from fees for FairX.
// The caller of convertMultiple, the function responsible for converting fees to FairX earns a 0.1% reward for calling.
contract BrewFairX is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    IFairdexV2Factory public immutable factory;
    ISwapper public immutable swapper;

    address public immutable xFair;
    address private immutable fairX;
    ...
    modifier onlyAuth() {
        require(isAuth[_msgSender()], "BrewFairX: FORBIDDEN");
        _;
    }
    ...
    modifier onlyEOA() {
        ...
        require(msg.sender == tx.origin, "BrewFairX: must use EOA");
        _;
    }
    ...
    constructor(
        ...
        factory = IFairdexV2Factory(_factory);
        xFair = _xFair;
        fairX = _fairX;
        wftm = _wftm;
        devAddr = msg.sender;
    ...
    function setBridgeRoute(uint index, address token) external onlyAuth {
        ...
        require(
            index <= bridgeRouteAmount,
            "index too large, use next free slot"
        );

        bridgeRoute[index] = token;
        if (index == bridgeRouteAmount) bridgeRouteAmount += 1;
    }
    ...
    function isLpToken(address _adr) internal returns (bool) {
        ...
        IFairdexV2Pair pair = IFairdexV2Pair(_adr);
    ...
    function setDevAddr(address _addr) external {
        require(
            owner() == _msgSender() || devAddr == _msgSender(),
            "not allowed"
        );
        require(
            _addr != address(0),
    ...
    function setBridge(address token, address bridge) external onlyAuth {
        // Checks
        require(
            token != fairX && token != wftm && token != bridge,
            "BrewFairX: Invalid bridge"
        );
    ...
    function convertMultiple(
        ...
        for (i = 0; i < token0.length; ) {
            ...
            require(
                !isLpToken(token0[i]) && !isLpToken(token1[i]),
                "no LP allowed"
            );
            pair = IFairdexV2Pair(_getPair(token0[i], token1[i]));
            require(address(pair) != address(0), "BrewFairX: Invalid pair");
        ...
        for (i = 0; i < token0.length; ) {
            if (block.number > converted[token0[i]]) {
                _convertStep(
                    token0[i],
                    IERC20(token0[i]).balanceOf(address(this))
                );
            ...
            if (block.number > converted[token1[i]]) {
                _convertStep(
                    token1[i],
                    IERC20(token1[i]).balanceOf(address(this))
                );
        ...
        // final step is to swap all wFTM to FairX and disperse it
        ...
        (, success) = _swap(wftm, fairX, i);
        if (!success) revert("BrewFairX: swap failure in toFairX");

        //disperse
        uint _amt = IERC20(fairX).balanceOf(address(this));
        uint bounty = _amt.mul(BOUNTY_FEE).div(10000);
        i = _amt.sub(bounty); //reuse i for amount to save gas
        IERC20(fairX).safeTransfer(xFair, i); // send xFair its share
        IERC20(fairX).safeTransfer(_msgSender(), bounty); // send message sender their share of 0.1%
        emit LogConvert(_msgSender(), fairX, _amt, i);
    }
    ...
    function _convertStep(
        ...
        if (token0 == wftm || token0 == fairX) {
            ...
            if (success) _convertStep(bridge, amount);
            else
                for (uint i = 0; i < bridgeRouteAmount; ) {
                    ...
                    if (!success)
                        if (i == bridgeRouteAmount - 1)
                            revert(
                                "BrewFairX: bridge route failure - all options exhausted"
                            );
                        else {
                            unchecked {
                                ++i;
    ...
    function _swap(
        ...
        if (fromToken == toToken) return (amountIn, false);
        ...
        try
            swapper.swap(
                fromToken,
                IFairdexV2Pair(_getPair(fromToken, toToken)),
                amountIn
            )
        returns (uint amount) {
            return (amount, true);
    ...
    //allows migration of lp tokens balance ONLY to the new(current) BrewFairX
