...
import "./interfaces/IAirdropERC20Claimable.sol";
...
contract AirdropFairXClaimable is
    Initializable,
    ...
    function verifyClaim(
        ...
        if (merkleRoot != bytes32(0)) {
            (isOverride, ) = MerkleProof.verify(
                ...
                merkleRoot,
                keccak256(
                    abi.encodePacked(_claimer, _proofMaxQuantityForWallet)
                )
            );
        }
        ...
        require(
            expTimestamp == 0 || block.timestamp < expTimestamp,
            "airdrop expired."
        );
        ...
        require(
            _quantity + supplyClaimedAlready <= claimLimitForWallet,
            "invalid quantity."
        );
    }
